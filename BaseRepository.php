...
abstract class BaseRepository extends \yii\base\Object
{
    ...
    public function saveImages($imageUrls) {
        ...
        if (!is_array($imageUrls)) {
            throw new Exception('imageUrls must be an array!');
        }
    ...
    public function searchByImage($imagePath, &$result) {
        ...
        if (!file_exists($imagePath)) {
            throw new Exception('file specified by parameter imagePath not exist!');
        }
     ...
     * @return boolean | array 请求是否成功,不成功返回false,成功返回结果数组
     */
    private function sendRequest($postData) {

        if (!is_array($postData)) {
            throw new Exception('postData must be an array!');
        }
        ...
        $result = [];
        if ($output === false) {
            throw new Exception('call idl service failed!');
        }
